...
public class TopUpInfoImpl extends Presenter<TopUpInfoView> implements TopUpInfo, OnNetworkLoadListener<ResponseMessage<TopUpInfoResponse>> {
    ...
    public void getInfo(Operator operator) {
        try {
            ...
            String strJson = new AESMessageEncryptor().encryptRequest(new Gson().toJson(request), KeyExchangerImpl.getKey(), KeyExchangerImpl.getIv(), KeyExchangerImpl.getEncId());
            new TopUpNetWorker(modelLayer, true, false).topUpInfo(strJson, this);
        } catch (Exception e) {
            e.printStackTrace();
            view.onError();
        }
